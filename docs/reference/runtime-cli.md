...
`euclid run --config examples/current_release_run.yaml` loads an `OperatorRequest`, writes `active-runs/<run_id>/control-plane/execution-state.sqlite3`, and seals replay artifacts under the run workspace. Replay is part of the product, not an afterthought: `euclid replay --run-id <run-id>` checks the recorded manifest, seed, artifact hash, and stage order before a publication surface is treated as complete.

Release commands use the canonical scope terms `current_release`, `full_vision`, and `shipped_releasable`. Demo commands remain compatibility-only tooling and are not a certified runtime.
