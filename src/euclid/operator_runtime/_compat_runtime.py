...
from euclid.operator_runtime.workflow import (
    CandidateSummary,
    OperatorReducerWorkflowResult,
    OperatorReplayResult,
    replay_operator_run,
    run_operator_reducer_workflow,
)
...
class DemoRunResult:
    ...
    workflow_result: OperatorReducerWorkflowResult
...
def load_demo_request(manifest_path: Path | None = None) -> DemoRequest:
    ...
    mechanistic_evidence_payload = _mechanistic_evidence_payload(
        payload.get("mechanistic_evidence"),
        field_path="mechanistic_evidence",
    )
    robustness_payload = _robustness_payload(
        payload.get("robustness"),
        field_path="robustness",
    )
    declared_entity_panel = _tuple_of_strings(
        payload.get("declared_entity_panel", ()),
    ...
    *,
    ...
    with FileLock(paths.run_lock_path):
        ...
        execution_state.upsert_worker_metadata(
            run_id=request.request_id,
            worker_id="operator.reducer.workflow",
            module_id="manifest_registry",
            status="running",
            details={"entrypoint_id": "operator.run"},
        )
        ...
        try:
            if _uses_shared_local_operator_workflow(request):
                ...
                workflow_result = run_operator_reducer_workflow(
                    ...
                    external_evidence_payload=(
                        None
                        if request.external_evidence_payload is None
                        else dict(request.external_evidence_payload)
                    ),
                    mechanistic_evidence_payload=(
                        None
                        if request.mechanistic_evidence_payload is None
                        else dict(request.mechanistic_evidence_payload)
                    ),
                    robustness_payload=(
                        None
                        if request.robustness_payload is None
                        else dict(request.robustness_payload)
                    ),
                    telemetry=telemetry,
                )
...
def profile_demo_run(
    *,
    ...
    telemetry_path = write_performance_telemetry(
        result.paths.output_root / "telemetry" / "operator-run-profile.json",
        artifact,
    )
...
def replay_demo(
    *,
    ...
    summary = DemoReplaySummary(
        ...
        forecast_object_type=str(summary_payload.get("forecast_object_type", "point")),
...
def run_demo_probabilistic_evaluation(
    *,
    ...
    with FileLock(paths.run_lock_path):
        ...
        if mechanistic_evidence is not None:
            scorecard_decision = resolve_scorecard_status(
                ...
                time_safety_status=str(
                    intake.time_safety_audit.manifest.body["status"]
                ),
                calibration_status=str(calibration_result.manifest.body["status"]),
                descriptive_failure_reason_codes=descriptive_failure_reason_codes,
                robustness_reason_codes=robustness_reason_codes,
        ...
        _write_summary_payload(
            ...
            payload={
                ...
                "forecast_object_type": request.forecast_object_type,
                "scope_ledger_ref": scope_ledger.manifest.ref.as_dict(),
                "selected_candidate_ref": reducer_artifact.manifest.ref.as_dict(),
    ...
    summary = DemoProbabilisticRunSummary(
        ...
        scope_ledger_ref=scope_ledger.manifest.ref,
        selected_candidate_ref=reducer_artifact.manifest.ref,
        ...
        abstention_ref=abstention.manifest.ref if abstention is not None else None,
        prediction_artifact_ref=prediction_artifact.manifest.ref,
        score_result_ref=score_result.manifest.ref,
        calibration_result_ref=calibration_result.manifest.ref,
        aggregated_primary_score=float(
            score_result.manifest.body["aggregated_primary_score"]
        ),
        calibration_status=str(calibration_result.manifest.body["status"]),
...
def _run_shared_local_point_workflow(
    *,
    ...
    selected_candidate_id = _candidate_source_id(selected_candidate)
    baseline_candidate = _realize_shared_local_panel_candidate(
        search_plan=intake.search_plan_object,
        feature_view=intake.feature_view_object,
        candidate_id="shared_local_panel_mean_offsets_baseline",
    )
    fit_result = fit_candidate_window(
        candidate=selected_candidate,
        feature_view=intake.feature_view_object,
        ...
        search_plan=intake.search_plan_object,
        stage_id="confirmatory_holdout",
        shared_local_backend_preference="optimizer",
    )
    baseline_fit_result = fit_candidate_window(
        candidate=baseline_candidate,
        feature_view=intake.feature_view_object,
        ...
        search_plan=intake.search_plan_object,
        stage_id="confirmatory_holdout",
        shared_local_backend_preference="baseline",
    )
...
def _external_evidence_payload(
    ...
    if value is None:
        return None
    payload = dict(_mapping_or_empty(value, field_path=field_path))
    _string_field(payload.get("bundle_id"), field_path=f"{field_path}.bundle_id")
    _string_field(payload.get("domain_id"), field_path=f"{field_path}.domain_id")
    _mapping_or_empty(
        payload.get("acquisition_window"),
        field_path=f"{field_path}.acquisition_window",
    )
    raw_sources = payload.get("raw_sources")
    if not isinstance(raw_sources, list) or not raw_sources:
        raise ValueError(f"{field_path}.raw_sources must be a non-empty list")
    for index, source in enumerate(raw_sources):
        if not isinstance(source, Mapping):
            raise ValueError(f"{field_path}.raw_sources[{index}] must be a mapping")
    return payload


def _mechanistic_evidence_payload(
    value: object, *, field_path: str
) -> dict[str, object] | None:
    if value is None:
        return None
    payload = dict(_mapping_or_empty(value, field_path=field_path))
    _string_field(
        payload.get("mechanistic_evidence_id"),
        field_path=f"{field_path}.mechanistic_evidence_id",
    )
    for key in ("term_bindings", "term_units", "invariance_checks"):
        entries = payload.get(key)
        if not isinstance(entries, list):
            raise ValueError(f"{field_path}.{key} must be a list")
        for index, entry in enumerate(entries):
            if not isinstance(entry, Mapping):
                raise ValueError(
                    f"{field_path}.{key}[{index}] must be a mapping"
                )
    return payload


def _robustness_payload(value: object, *, field_path: str) -> dict[str, object] | None:
    if value is None:
        return None
    payload = dict(_mapping_or_empty(value, field_path=field_path))
    forced_failures = payload.get("forced_leakage_canary_failures", ())
    if not isinstance(forced_failures, (list, tuple)):
        raise ValueError(
            f"{field_path}.forced_leakage_canary_failures must be a list"
        )
    for index, canary_type in enumerate(forced_failures):
        _string_field(
            canary_type,
            field_path=f"{field_path}.forced_leakage_canary_failures[{index}]",
        )
    return payload
...
def _build_search_intake(
    *,
    ...
    return build_operator_intake_plan(
        csv_path=request.dataset_csv,
        catalog=catalog,
        registry=registry,
        cutoff_available_at=request.cutoff_available_at,
...
def _register_requested_mechanistic_evidence(
    *,
    catalog: ContractCatalog,
    registry: ManifestRegistry,
    request: DemoRequest,
    ...
    if intake.external_evidence_bundle is None:
        raise ContractValidationError(
            ...
            message=(
                "mechanistic evidence requires an operator-native external evidence "
                "bundle"
            ),
            field_path="mechanistic_evidence",
        )
    ...
    return register_mechanistic_evidence(
        catalog=catalog,
        registry=registry,
        mechanistic_evidence_id=str(payload["mechanistic_evidence_id"]),
        ...
        term_bindings=tuple(
            dict(item)
            for item in payload.get("term_bindings", ())
            if isinstance(item, Mapping)
        ),
        term_units=tuple(
            dict(item)
            for item in payload.get("term_units", ())
            if isinstance(item, Mapping)
        ),
        invariance_checks=tuple(
            dict(item)
            for item in payload.get("invariance_checks", ())
            if isinstance(item, Mapping)
        ),
...
def _robustness_reason_codes(
    *,
    ...
    if isinstance(null_result_ref, Mapping):
        null_result = registry.resolve(_coerce_typed_ref(null_result_ref)).manifest.body
    ...
    for ref in robustness_report_body.get("perturbation_family_result_refs", ()):
        if isinstance(ref, Mapping):
            perturbation_family_results.append(
                registry.resolve(_coerce_typed_ref(ref)).manifest.body
            )

    for family_result in perturbation_family_results:
        if str(family_result.get("status")) == "failed":
            codes.append("perturbation_protocol_failed")
        for metric in family_result.get("metric_results", ()):
    ...
    canary_results: list[Mapping[str, Any]] = [
        item
        for item in robustness_report_body.get("leakage_canary_results", ())
        if isinstance(item, Mapping)
    ]
    for ref in robustness_report_body.get("leakage_canary_result_refs", ()):
        if isinstance(ref, Mapping):
            canary_results.append(registry.resolve(_coerce_typed_ref(ref)).manifest.body)

    canary_failure_codes: list[str] = []
    for canary_result in canary_results:
        failure_reason_codes = canary_result.get("failure_reason_codes", ())
        if isinstance(failure_reason_codes, (list, tuple)):
            canary_failure_codes.extend(str(code) for code in failure_reason_codes)
        if not bool(canary_result.get("pass", False)) or canary_failure_codes:
...
def _register_probabilistic_robustness_artifacts(
    *,
    ...
    forced_leakage_canary_failures = set(
        _forced_leakage_canary_failures(request.robustness_payload)
    )
    ...
    for canary_type in (
        ...
        downstream_evidence_refs: tuple[Mapping[str, Any], ...] = ()
        observed_terminal_state = "blocked_at_expected_stage"
        if canary_type in forced_leakage_canary_failures:
            observed_terminal_state = "survived_to_scoring"
            downstream_evidence_refs = (
                {
                    "ref_kind": "search_plan",
                    "typed_ref": intake.search_plan.manifest.ref.as_dict(),
                },
                {
                    "ref_kind": "feature_view",
                    "typed_ref": intake.feature_view.manifest.ref.as_dict(),
                },
            )
        canary_result = registry.register(
            build_leakage_canary_result_manifest(
                ...
                downstream_evidence_refs=downstream_evidence_refs,
            ),
            parent_refs=(
                canary.manifest.ref,
                TypedRef(
                ...
                *tuple(
                    _coerce_typed_ref(ref["typed_ref"])
                    for ref in downstream_evidence_refs
                ),
            ),
        )
            ...
            perturbation_runs=(
                {
                    ...
                    "description_gain_bits": _stable_demo_float(
                        description_gain_bits
                    ),
                    "failure_reason_code": None,
...
def _demo_paths(
    *,
    ...
    effective_request_id = (
        request.request_id if request else (request_id or "operator-run")
    )
    ...
    *,
    ...
    _write_summary_payload(
        ...
        payload={
            ...
            "scope_ledger_ref": workflow_result.scope_ledger.manifest.ref.as_dict(),
            "publication_record_ref": (
                workflow_result.publication_record.manifest.ref.as_dict()
            ),
            "selected_candidate_ref": summary.selected_candidate_ref.as_dict(),
            "selected_candidate_id": str(
                workflow_result.selected_candidate.manifest.body["candidate_id"]
            ),
            "selected_family": summary.selected_family,
            "forecast_object_type": request.forecast_object_type,
    ...
    *,
    ...
    for summary_path in summary_files:
        ...
        if selected_summary_path is not None and run_id is None:
            raise ValueError(
                "multiple operator runs found under output_root; "
    ...
    if selected_summary_path is None or selected_summary is None:
        raise FileNotFoundError(
            f"no operator run summary found under {output_root} for run_id {run_id!r}"
        )
...
def load_operator_request(manifest_path: Path | None = None) -> DemoRequest:
    return load_demo_request(manifest_path)


def run_operator_point(
    *,
    manifest_path: Path | None = None,
    output_root: Path | None = None,
    telemetry: TelemetryRecorder | None = None,
...
def run_operator_probabilistic(
    *,
    manifest_path: Path | None = None,
    output_root: Path | None = None,
) -> DemoProbabilisticEvaluationResult:
    return run_demo_probabilistic_evaluation(
        manifest_path=manifest_path,
        output_root=output_root,
    )


def replay_operator_bundle(
    *,
    output_root: Path | None = None,
    run_id: str | None = None,
    bundle_ref: str | TypedRef | None = None,
) -> DemoReplayResult:
    return replay_demo(output_root=output_root, run_id=run_id, bundle_ref=bundle_ref)
...
def profile_operator_run(
    *,
    ...
    return profile_demo_run(manifest_path=manifest_path, output_root=output_root)
