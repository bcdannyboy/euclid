...
Euclid's public authority surface is this reference workspace plus the schema and release contracts it names. The system does not publish universal-law language. A stronger claim remains bounded by the claim lane, forecast object type, validation scope, replay evidence, and readiness policy attached to the run.

The active subsystem labels are runtime and control plane, modeling pipeline, search and model semantics, formal specification and release evidence, and local analysis workbench. The code anchors are `src/euclid/cli`, `src/euclid/modules`, `src/euclid/search`, `src/euclid/contracts`, and `src/euclid/workbench`.
