...
from euclid.math.operator_support import build_operator_support_bundle
...
from euclid.modules.residual_diagnostics import (
...
from euclid.operator_runtime.intake_planning import (
...
_REPLAY_ENTRYPOINT_ID = "operator_native_replay_v1"
...
@dataclass(frozen=True)
class OperatorReducerWorkflowResult:
    intake: OperatorIntakePlanningResult
...
class _CandidateEvaluation:
    ...
    development_prediction_rows: tuple[dict[str, Any], ...] = ()


@dataclass(frozen=True)
class _ResidualSequenceSignal:
    sample_count: int
    ar1_coefficient: float | None
    ar1_mae_ratio: float | None
    ar1_r_squared: float | None
    near_zero_residual: bool
...
def run_operator_reducer_workflow(
    *,
    csv_path,
    ...
    mechanistic_evidence_payload: dict[str, object] | None = None,
    robustness_payload: dict[str, object] | None = None,
    declared_entity_panel: tuple[str, ...] = (),
) -> OperatorReducerWorkflowResult:
    intake = build_operator_intake_plan(
        ...
        declared_entity_panel=declared_entity_panel,
    )
    ...
    scope_ledger = registry.register(
        _build_manifest(
            ...
            body={
                "scope_ledger_id": "operator_scope_ledger_v1",
                "scope_id": _SCOPE_ID,
    ...
    point_score_policy = registry.register(
        _build_manifest(
            ...
            module_id="scoring",
            body={
                "score_policy_id": "operator_absolute_error_policy_v1",
    ...
    if telemetry is not None:
        telemetry.record_measurement(
            ...
            unit="families",
            attributes={"scope": "operator_workflow"},
        )
    best_descriptive_candidate_runtime = _select_best_overall_candidate(
    ...
    forward_validation_candidate_runtime = (
        accepted_candidate_runtime or best_descriptive_candidate_runtime
    )
    candidate_roles = _candidate_roles_surface(
        best_descriptive_candidate=best_descriptive_candidate_runtime,
        accepted_candidate=accepted_candidate_runtime,
        forward_validation_candidate=forward_validation_candidate_runtime,
    )
    residual_diagnostics = _residual_diagnostics_surface(
        accepted_candidate=accepted_candidate_runtime
    )
    ...
    selected_candidate_spec = _register_runtime_model(
        ...
        catalog,
        CandidateSpecManifest(
            candidate_spec_id=f"{forward_validation_candidate_runtime.candidate_id}_spec",
            family_id=forward_validation_candidate_runtime.family_id,
            parameter_summary=_normalize_mapping(
                forward_validation_candidate_runtime.parameters
            ),
        ...
        _build_manifest(
            ...
            body={
                "canonical_structure_code_id": (
                    f"{forward_validation_candidate_runtime.candidate_id}_structure"
                ),
                "owner_id": "module.candidate-fitting-v1",
                "scope_id": _SCOPE_ID,
                "candidate_id": forward_validation_candidate_runtime.candidate_id,
                "family_id": forward_validation_candidate_runtime.family_id,
                "structure_signature": (
                    forward_validation_candidate_runtime.structure_signature
                ),
            },
        ),
    ...
    selected_candidate = registry.register(
        _build_manifest(
            ...
            module_id="candidate_fitting",
            body={
                "reducer_artifact_id": forward_validation_candidate_runtime.candidate_id,
                "owner_id": "module.candidate-fitting-v1",
                "scope_id": _SCOPE_ID,
                "candidate_id": forward_validation_candidate_runtime.candidate_id,
                "family_id": forward_validation_candidate_runtime.family_id,
                ...
                "description_gain_bits": _stable_float(
                    forward_validation_candidate_runtime.description_gain_bits
                ),
                "exploratory_primary_score": _stable_float(
                    forward_validation_candidate_runtime.exploratory_primary_score
                ),
                "candidate_spec_ref": selected_candidate_spec.manifest.ref.as_dict(),
                "candidate_roles": candidate_roles,
                "best_descriptive_candidate_id": (
                    best_descriptive_candidate_runtime.candidate_id
                ),
                "accepted_candidate_id": (
                    None
                    if accepted_candidate_runtime is None
                    else accepted_candidate_runtime.candidate_id
                ),
    ...
    search_ledger = _register_runtime_model(
        ...
        build_search_ledger(
            ...
            selected_candidate_id=best_descriptive_candidate_runtime.candidate_id,
            accepted_candidate_id=(
    ...
    freeze_chain = _freeze_chain_surface(
    ...
    prediction_artifact = _register_runtime_model(
        ...
        PredictionArtifactManifest(
            prediction_artifact_id=(
                f"{forward_validation_candidate_runtime.candidate_id}_confirmatory_prediction"
            ),
            candidate_id=forward_validation_candidate_runtime.candidate_id,
            ...
            rows=tuple(
                ...
                for index, row in enumerate(
                    forward_validation_candidate_runtime.confirmatory_prediction_rows
                )
            ),
            parent_refs=(
                selected_candidate.manifest.ref,
    ...
    point_score_result = _register_runtime_model(
        ...
        catalog,
        PointScoreResultManifest(
            score_result_id=f"{forward_validation_candidate_runtime.candidate_id}_point_score",
            score_policy_ref=point_score_policy.manifest.ref,
    ...
    calibration_contract = registry.register(
        _build_manifest(
            ...
            module_id="scoring",
            body={
                "calibration_contract_id": "operator_point_calibration_contract_v1",
                "owner_prompt_id": "prompt.scoring-calibration-v1",
                "scope_id": _SCOPE_ID,
                "forecast_object_type": "point",
    ...
    calibration_result = registry.register(
        _build_manifest(
            ...
            module_id="scoring",
            body={
                "calibration_result_id": "operator_point_calibration_result_v1",
                "owner_prompt_id": "prompt.scoring-calibration-v1",
                "scope_id": _SCOPE_ID,
    ...
    comparison_universe = registry.register(
        build_comparison_universe(
            selected_candidate_id=forward_validation_candidate_runtime.candidate_id,
    ...
    run_result_id = _run_result_id(forward_validation_candidate_runtime)
    bundle_id = _bundle_id(forward_validation_candidate_runtime)
    scorecard_object_id = f"{run_result_id}_scorecard"
    validation_scope_object_id = f"{run_result_id}_validation_scope"
    claim_card_object_id = f"{run_result_id}_claim_card"
    run_result_ref = TypedRef(
    ...
    scorecard_ref = TypedRef(
        ...
        object_id=scorecard_object_id,
    )
    validation_scope_ref = TypedRef(
        schema_name="validation_scope_manifest@1.0.0",
        object_id=validation_scope_object_id,
    )
    claim_card_ref = TypedRef(
        schema_name="claim_card_manifest@1.1.0",
        object_id=claim_card_object_id,
    )
    ...
    robustness_artifacts = _materialize_robustness_artifacts(
        ...
        frozen_shortlist=frozen_shortlist,
        selected_candidate_runtime=forward_validation_candidate_runtime,
        minimum_description_gain_bits=minimum_description_gain_bits,
        forced_leakage_canary_failures=_forced_leakage_canary_failures(
            robustness_payload
        ),
    )
    ...
    scorecard_decision = resolve_scorecard_status(
        candidate_admissible=forward_validation_candidate_runtime.admissible,
    ...
    if mechanistic_evidence_payload is not None:
        if intake.external_evidence_bundle is None:
            raise ContractValidationError(
                code="mechanistic_external_evidence_required",
                message=(
                    "mechanistic evidence requires an operator-native external "
                    "evidence bundle"
                ),
        ...
        mechanistic_evidence = register_mechanistic_evidence(
            ...
            mechanistic_evidence_id=str(
                mechanistic_evidence_payload["mechanistic_evidence_id"]
            ),
            candidate_ref=selected_candidate.manifest.ref,
            prediction_artifact_ref=prediction_artifact.manifest.ref,
            external_evidence_ref=intake.external_evidence_bundle.bundle.manifest.ref,
            lower_claim_ceiling=lower_claim_ceiling,
            term_bindings=tuple(
                dict(item)
                for item in mechanistic_evidence_payload.get("term_bindings", ())
                if isinstance(item, Mapping)
            ),
            term_units=tuple(
                dict(item)
                for item in mechanistic_evidence_payload.get("term_units", ())
                if isinstance(item, Mapping)
            ),
            invariance_checks=tuple(
                dict(item)
                for item in mechanistic_evidence_payload.get(
                    "invariance_checks",
                    (),
                )
                if isinstance(item, Mapping)
            ...
            external_evidence_records=tuple(
                dict(record.manifest.body)
                for record in intake.external_evidence_bundle.records
            ),
        ...
        scorecard_decision = resolve_scorecard_status(
            candidate_admissible=forward_validation_candidate_runtime.admissible,
            robustness_status=str(
                robustness_report.manifest.body.get(
            ...
            mechanistic_reason_codes=tuple(
                str(code)
                for code in mechanistic_evidence.dossier.manifest.body.get(
                    "reason_codes",
                    (),
                )
            ),
    ...
    scorecard = registry.register(
        _build_manifest(
            ...
            object_id=scorecard_object_id,
            body={
                "scorecard_id": "operator_scorecard_v1",
                ...
                "L_family_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components["L_family_bits"]
                ),
                "L_structure_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components[
                        "L_structure_bits"
                    ]
                ),
                "L_literals_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components["L_literals_bits"]
                ),
                "L_params_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components["L_params_bits"]
                ),
                "L_state_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components["L_state_bits"]
                ),
                "L_data_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components["L_data_bits"]
                ),
                "L_total_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components["L_total_bits"]
                ),
                "reference_bits": _stable_float(
                    forward_validation_candidate_runtime.description_components["reference_bits"]
                ),
                "description_gain_bits": _stable_float(
                    forward_validation_candidate_runtime.description_gain_bits
                ),
                ...
                "mechanistic_status": scorecard_decision.mechanistic_status,
                "mechanistic_reason_codes": list(
                    scorecard_decision.mechanistic_reason_codes
                ),
                "candidate_roles": candidate_roles,
                "freeze_chain": freeze_chain,
                "residual_diagnostics": residual_diagnostics,
                "residual_law": None,
                "predictive_law": _predictive_law_projection(
                    publishable=(
                        scorecard_decision.descriptive_status == "passed"
                        and scorecard_decision.predictive_status == "passed"
                    ),
                ...
                **(
                    ...
                    if mechanistic_evidence is None
                        ...
                        "mechanistic_evidence_ref": (
                            mechanistic_evidence.dossier.manifest.ref.as_dict()
                        ),
                    }
                ),
            },
        ...
        parent_refs=(
            ...
            *(
                ()
                if mechanistic_evidence is None
                else (mechanistic_evidence.dossier.manifest.ref,)
            ),
        ),
    )
    ...
    external_evidence = (
        None
        if intake.external_evidence_bundle is None
        else intake.external_evidence_bundle.bundle
    )
    if telemetry is None:
        ...
        validation_scope = registry.register(
            _build_manifest(
                ...
                object_id=validation_scope_object_id,
                body={
                    "validation_scope_id": "operator_validation_scope_v1",
                    ...
                    "residual_law": None,
                    "predictive_law": _predictive_law_projection(
                        publishable=_claim_decision_publishable_predictive_law(
        ...
        if publication_mode == "candidate_publication":
            claim_card = registry.register(
                _build_manifest(
                    ...
                    object_id=claim_card_object_id,
                    body={
                        "claim_card_id": "operator_claim_card_v1",
                        "candidate_ref": selected_candidate.manifest.ref.as_dict(),
            ...
            abstention = _register_runtime_model(
                ...
                catalog,
                AbstentionManifest(
                    abstention_id="operator_abstention_v1",
        ...
        run_result_manifest = build_run_result_manifest(
            ...
            primary_external_evidence_ref=(
                external_evidence.manifest.ref
                if external_evidence is not None
                else None
            ),
            primary_mechanistic_evidence_ref=(
                mechanistic_evidence.dossier.manifest.ref
            ...
            build_reproducibility_bundle_manifest(
                ...
                required_manifest_refs=required_manifest_refs_for_publication(
                    ...
                    supporting_refs=(
                        ...
                        *(
                            ...
                            else (external_evidence.manifest.ref,)
                        ),
                        *(
                            ()
                            if mechanistic_evidence is None
                ...
                artifact_hash_records=build_artifact_hash_records(
                    ...
                    mechanistic_evidence=(
                        None
                        if mechanistic_evidence is None
                        else mechanistic_evidence.dossier
                    ),
                ),
        ...
        run_result = _register_runtime_model(
            ...
            build_run_result_manifest(
                ...
                primary_external_evidence_ref=(
                    run_result_manifest.primary_external_evidence_ref
                ),
                primary_mechanistic_evidence_ref=(
                    run_result_manifest.primary_mechanistic_evidence_ref
                ),
        ...
        readiness_judgment = _register_runtime_model(
            ...
            _build_runtime_readiness_manifest(
                judgment_id="operator_readiness_judgment_v1",
                replay_verification_status="verified",
        ...
        schema_lifecycle_integration_closure = _register_runtime_model(
            ...
            catalog,
            SchemaLifecycleIntegrationClosureManifest(
                closure_id="operator_schema_lifecycle_integration_closure_v1",
                status="passed",
            ),
        )
    ...
    if replay.replay_verification_status != "verified":
        raise ValueError(
            "replay verification failed for the sealed operator reducer workflow"
        )

    return OperatorReducerWorkflowResult(
        intake=intake,
        ...
        confirmatory_primary_score=forward_validation_candidate_runtime.confirmatory_primary_score,
        replay_verified=True,
    )


def replay_operator_run(
    *,
    bundle_ref: TypedRef,
    catalog: ContractCatalog,
    registry: ManifestRegistry,
    ...
    replay_accepted_candidate = _select_accepted_candidate(candidates)
    replay_candidate = replay_accepted_candidate or _select_best_overall_candidate(
        candidates
    )
    residual_diagnostics = _residual_diagnostics_surface(
        accepted_candidate=replay_accepted_candidate
    )
    scorecard_decision = resolve_scorecard_status(
    ...
    if isinstance(
        run_result.manifest.body.get("primary_external_evidence_ref"),
        Mapping,
    ):
        replayed_stage_order.append("external_evidence_resolved")
    if isinstance(
        run_result.manifest.body.get("primary_mechanistic_evidence_ref"), Mapping
    ):
        replayed_stage_order.append("mechanistic_evidence_resolved")
    replayed_stage_order.extend(
        ["publication_decision_resolved", "run_result_assembled"]
    )
    verification = verify_replayed_outcome(
        ...
        outcome=ReplayedOutcome(
            ...
            replayed_stage_order=tuple(replayed_stage_order),
...
def _evaluate_candidate_family(
    *,
    ...
    if telemetry is None:
        ...
        with telemetry.span(
            f"operator.fitting.development.{family_id}",
            category="fitting",
            ...
            full_fit = _fit_candidate(family_id, development_targets)
        with telemetry.span(
            f"operator.fitting.confirmatory.{family_id}",
            category="fitting",
    ...
    if telemetry is None:
        ...
        with telemetry.span(
            f"operator.evaluation.development.{family_id}",
            category="evaluation",
            attributes={"family_id": family_id, "phase": "development"},
        ):
            for test_rows, fit in development_fold_materials:
        ...
        with telemetry.span(
            f"operator.evaluation.confirmatory.{family_id}",
            category="evaluation",
            attributes={"family_id": family_id, "phase": "confirmatory"},
        ):
            confirmatory_predictions = _predict_rows(
    ...
    return _CandidateEvaluation(
        family_id=family_id,
        candidate_id=f"operator_{family_id}_candidate_v1",
        parameters=full_fit["parameters"],
        ...
        development_prediction_rows=tuple(development_prediction_rows),
    )
...
def _materialize_robustness_artifacts(
    *,
    ...
    minimum_description_gain_bits: float,
    forced_leakage_canary_failures: tuple[str, ...] = (),
) -> _RobustnessArtifacts:
    surrogate_generator = registry.register(
        build_surrogate_generator_manifest(
            catalog,
            surrogate_generator_id="operator_surrogate_generator_v1",
        )
    )
    null_protocol = registry.register(
        build_null_protocol_manifest(
            catalog,
            protocol_id="operator_null_protocol_v1",
    ...
    retention_metrics = {
        "canonical_form_exact_match_rate": registry.register(
            build_retention_metric_manifest(
                catalog,
                retention_metric_id="operator_canonical_form_exact_match_rate_v1",
        ...
        "positive_description_gain_rate": registry.register(
            build_retention_metric_manifest(
                catalog,
                retention_metric_id="operator_positive_description_gain_rate_v1",
        ...
        "outer_baseline_win_rate": registry.register(
            build_retention_metric_manifest(
                catalog,
                retention_metric_id="operator_outer_baseline_win_rate_v1",
    ...
    perturbation_protocol = registry.register(
        build_perturbation_protocol_manifest(
            catalog,
            protocol_id="operator_perturbation_protocol_v1",
    ...
    null_result_manifest = _register_runtime_model(
        ...
        build_null_result_manifest(
            catalog,
            null_result_id="operator_null_result_v1",
        ...
        _register_runtime_model(
            ...
            build_perturbation_family_result_manifest(
                ...
                perturbation_family_result_id=(
                    f"operator_{evaluation.family_id}_perturbation_result_v1"
                ),
    ...
    canary_manifests = tuple(
        registry.register(
            build_leakage_canary_manifest(
                catalog,
                canary_id=f"operator_{canary_type}_v1",
        ...
        registry.register(
            build_leakage_canary_result_manifest(
                ...
                observed_terminal_state=(
                    "survived_to_scoring"
                    if str(canary.manifest.body["canary_type"])
                    in forced_leakage_canary_failures
                    else "blocked_at_expected_stage"
                ),
                ...
                downstream_evidence_refs=(
                    ()
                    if str(canary.manifest.body["canary_type"])
                    not in forced_leakage_canary_failures
                    else (
                        {
                            "ref_kind": "frozen_shortlist",
                            "typed_ref": frozen_shortlist.manifest.ref.as_dict(),
                        },
                    )
                ),
            )
    ...
    robustness_report = _register_runtime_model(
        ...
        build_robustness_report(
            ...
            report_id="operator_robustness_report_v1",
            parent_refs=(
                frozen_shortlist.manifest.ref,
                intake.evaluation_plan.manifest.ref,
...
def _candidate_roles_surface(
    *,
    best_descriptive_candidate: _CandidateEvaluation,
    accepted_candidate: _CandidateEvaluation | None,
    forward_validation_candidate: _CandidateEvaluation,
) -> dict[str, Any]:
    return {
        "best_descriptive_candidate": _candidate_surface(best_descriptive_candidate),
        "accepted_candidate": (
            None
            if accepted_candidate is None
            else _candidate_surface(accepted_candidate)
        ),
        "forward_validation_candidate": _candidate_surface(
            forward_validation_candidate
        ),
        "selection_relation": _selection_relation(
            best_descriptive_candidate=best_descriptive_candidate,
            accepted_candidate=accepted_candidate,
            forward_validation_candidate=forward_validation_candidate,
        ),
    }


def _candidate_surface(candidate: _CandidateEvaluation) -> dict[str, Any]:
    return {
        "candidate_id": candidate.candidate_id,
        "family_id": candidate.family_id,
        "admissible": candidate.admissible,
        "description_gain_bits": _stable_float(candidate.description_gain_bits),
        "confirmatory_primary_score": _stable_float(
            candidate.confirmatory_primary_score
        ),
        "baseline_primary_score": _stable_float(candidate.baseline_primary_score),
    }
...
def _selection_relation(
    *,
    ...
    if best_descriptive_candidate.candidate_id == accepted_candidate.candidate_id:
        return "best_descriptive_is_accepted_candidate"
    if accepted_candidate.candidate_id == forward_validation_candidate.candidate_id:
        return "best_descriptive_separate_from_accepted_candidate"
    return "forward_validation_candidate_diverged"


def _freeze_chain_surface(
    *,
    selected_candidate: RegisteredManifest,
    selected_candidate_spec: RegisteredManifest,
    selected_candidate_structure: RegisteredManifest,
    ...
    return {
        "selected_candidate_ref": selected_candidate.manifest.ref.as_dict(),
        "selected_candidate_spec_ref": selected_candidate_spec.manifest.ref.as_dict(),
        "selected_candidate_structure_ref": (
            selected_candidate_structure.manifest.ref.as_dict()
        ),
        "frozen_shortlist_ref": frozen_shortlist.manifest.ref.as_dict(),
        "freeze_event_ref": freeze_event.manifest.ref.as_dict(),
    }


def _residual_diagnostics_surface(
    *,
    accepted_candidate: _CandidateEvaluation | None,
) -> dict[str, Any]:
    if accepted_candidate is None:
        return {
            ...
            "reason_codes": ["accepted_candidate_required"],
        }
    diagnostics = evaluate_residual_diagnostics(
        _residual_diagnostics_evidence(candidate=accepted_candidate)
    )
    return {
        "status": diagnostics.status,
        "residual_law_search_eligible": diagnostics.residual_law_search_eligible,
        "finite_dimensionality_status": diagnostics.finite_dimensionality_status,
        "recoverability_status": diagnostics.recoverability_status,
        "reason_codes": list(diagnostics.reason_codes),
    }


def _residual_diagnostics_evidence(
    *,
    candidate: _CandidateEvaluation,
) -> ResidualDiagnosticsEvidence:
    prediction_rows = tuple(candidate.confirmatory_prediction_rows)
    signal = _residual_sequence_signal(prediction_rows)
    replicated_signals = _replicated_residual_signals(prediction_rows)

    finite_dimensionality_supported = (
        signal.ar1_r_squared is not None and signal.ar1_r_squared >= 0.6
    )
    recoverability_supported = (
        signal.ar1_mae_ratio is not None and signal.ar1_mae_ratio <= 0.6
    )

    finite_dimensionality_replicated = _signals_replicated(
        replicated_signals=replicated_signals,
        requirement=lambda item: (
            item.ar1_r_squared is not None and item.ar1_r_squared >= 0.6
        ),
        expect_supported=finite_dimensionality_supported,
    )
    recoverability_replicated = _signals_replicated(
        replicated_signals=replicated_signals,
        requirement=lambda item: (
            item.ar1_mae_ratio is not None and item.ar1_mae_ratio <= 0.6
        ),
        expect_supported=recoverability_supported,
    )

    return ResidualDiagnosticsEvidence(
        finite_dimensionality=FiniteDimensionalityEvidence(
            sample_count=signal.sample_count,
            dimension_upper_bound=1 if finite_dimensionality_supported else None,
            replicated_across_splits=finite_dimensionality_replicated,
        ),
        recoverability=RecoverabilityEvidence(
            sample_count=signal.sample_count,
            state_reconstruction_passed=(
                True if recoverability_supported else None
            ),
            replicated_across_splits=recoverability_replicated,
        ),
    )


def _residual_sequence_signal(
    prediction_rows: tuple[dict[str, Any], ...],
) -> _ResidualSequenceSignal:
    residuals = _prediction_residuals(prediction_rows)
    sample_count = len(residuals)
    if sample_count < 2:
        return _ResidualSequenceSignal(
            sample_count=sample_count,
            ar1_coefficient=None,
            ar1_mae_ratio=None,
            ar1_r_squared=None,
            near_zero_residual=all(abs(value) <= 1e-9 for value in residuals),
        )
    xs = residuals[:-1]
    ys = residuals[1:]
    zero_baseline_mae = fmean(abs(value) for value in ys)
    if zero_baseline_mae <= 1e-9:
        return _ResidualSequenceSignal(
            sample_count=sample_count,
            ar1_coefficient=0.0,
            ar1_mae_ratio=0.0,
            ar1_r_squared=1.0,
            near_zero_residual=True,
        )
    denominator = sum(value * value for value in xs)
    coefficient = (
        0.0
        if denominator <= 1e-9
        else _stable_float(sum(x * y for x, y in zip(xs, ys, strict=True)) / denominator)
    )
    predicted = tuple(coefficient * value for value in xs)
    mae_ratio = _stable_float(
        fmean(abs(actual - guess) for actual, guess in zip(ys, predicted, strict=True))
        / zero_baseline_mae
    )
    mean_y = fmean(ys)
    variance = fmean((value - mean_y) ** 2 for value in ys)
    if variance <= 1e-9:
        r_squared = 1.0 if mae_ratio <= 1e-9 else 0.0
    else:
        mse = fmean(
            (actual - guess) ** 2
            for actual, guess in zip(ys, predicted, strict=True)
        )
        r_squared = _stable_float(1.0 - (mse / variance))
    return _ResidualSequenceSignal(
        sample_count=sample_count,
        ar1_coefficient=coefficient,
        ar1_mae_ratio=mae_ratio,
        ar1_r_squared=r_squared,
        near_zero_residual=False,
    )
...
def _prediction_residuals(
    ...
    return tuple(
        _stable_float(
            float(prediction["realized_observation"])
...
def _replicated_residual_signals(
    prediction_rows: tuple[dict[str, Any], ...],
) -> tuple[_ResidualSequenceSignal, ...]:
    if len(prediction_rows) < 4:
        return ()
    midpoint = len(prediction_rows) // 2
    if midpoint < 2 or len(prediction_rows) - midpoint < 2:
        return ()
    first = _residual_sequence_signal(prediction_rows[:midpoint])
    second = _residual_sequence_signal(prediction_rows[midpoint:])
    return (first, second)


def _signals_replicated(
    *,
    replicated_signals: tuple[_ResidualSequenceSignal, ...],
    requirement,
    expect_supported: bool,
) -> bool:
    if len(replicated_signals) < 2:
        return False
    return expect_supported and all(requirement(signal) for signal in replicated_signals)


def _apply_residual_predictive_gate(
    *,
    scorecard_decision: Any,
    residual_diagnostics: Mapping[str, Any],
) -> Any:
    if str(residual_diagnostics.get("status")) != "structured_residual_remains":
        return scorecard_decision
    return type(scorecard_decision)(
        descriptive_status=scorecard_decision.descriptive_status,
        descriptive_reason_codes=tuple(scorecard_decision.descriptive_reason_codes),
        predictive_status="blocked",
        predictive_reason_codes=_surface_reason_codes(
            tuple(scorecard_decision.predictive_reason_codes),
            ("structured_residual_remains",),
        ),
        mechanistic_status=(
            "not_requested"
            if str(scorecard_decision.mechanistic_status) == "not_requested"
            else "blocked_predictive_floor"
        ),
        mechanistic_reason_codes=(
            ()
            if str(scorecard_decision.mechanistic_status) == "not_requested"
            else ("predictive_floor_required",)
        ),
    )


def _claim_decision_publishable_predictive_law(
    *,
...
def _predictive_law_projection(
    *,
    ...
    return {
        ...
        "claim_card_ref": claim_card_ref.as_dict(),
        "scorecard_ref": scorecard_ref.as_dict(),
        "validation_scope_ref": validation_scope_ref.as_dict(),
        "publication_record_ref": publication_record_ref.as_dict(),
    }


def _surface_reason_codes(*reason_groups: tuple[str, ...]) -> tuple[str, ...]:
    ordered_reason_codes: list[str] = []
    for reason_group in reason_groups:
        for reason_code in reason_group:
            text = str(reason_code)
            if text and text not in ordered_reason_codes:
                ordered_reason_codes.append(text)
...
def _forced_leakage_canary_failures(
    robustness_payload: Mapping[str, object] | None,
) -> tuple[str, ...]:
    if robustness_payload is None:
        return ()
    forced_failures = robustness_payload.get("forced_leakage_canary_failures", ())
    if not isinstance(forced_failures, (list, tuple)):
        return ()
    return tuple(
        dict.fromkeys(
            str(canary_type)
            for canary_type in forced_failures
            if str(canary_type).strip()
        )
    )
...
def _run_result_id(candidate: _CandidateEvaluation) -> str:
    ...
    return f"operator_{candidate.family_id}_{suffix}_run_result_v1"


def _bundle_id(candidate: _CandidateEvaluation) -> str:
    suffix = "candidate" if candidate.admissible else "abstention"
    return f"operator_{candidate.family_id}_{suffix}_bundle_v1"
