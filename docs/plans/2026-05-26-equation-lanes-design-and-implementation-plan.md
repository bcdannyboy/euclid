...
```json
{
  ...
  "descriptive_exact": {
    ...
    "equation": {
      "label": "\\hat{y}(t_n)=0.5 + 0.2\\cdot \\cos\\left(\\frac{2\\pi \\cdot 1 \\cdot n}{5}\\right) - 0.1\\cdot \\sin\\left(\\frac{2\\pi \\cdot 1 \\cdot n}{5}\\right),\\quad n=0,\\ldots,4",
      "formula_latex": "\\hat{y}(t_n)=0.5 + 0.2\\cdot \\cos\\left(\\frac{2\\pi \\cdot 1 \\cdot n}{5}\\right) - 0.1\\cdot \\sin\\left(\\frac{2\\pi \\cdot 1 \\cdot n}{5}\\right),\\quad n=0,\\ldots,4",
      "candidate_id": "descriptive_exact_fourier_reconstruction",
      "family_id": "spectral",
...
| 8 | Formula-specific final pass | Completed | `src/euclid/workbench/service.py`, `tests/unit/workbench/test_service.py`, `tests/frontend/workbench/app.test.js`, `README.md`, `docs/workbench.md`, `docs/reference/workbench.md`, `docs/assets/readme/workbench/live-spy-30d/*` | RED: `PYTHONPATH=src python3.11 -m pytest -q tests/unit/workbench/test_service.py -k "descriptive_exact_reconstruction_exposes"` failed on the old placeholder label and missing `formula_latex`; GREEN: `PYTHONPATH=src python3.11 -m pytest -q tests/unit/workbench/test_service.py tests/integration/test_workbench_analysis.py` passed `114 passed`; `PYTHONPATH=src python3.11 -m pytest -q tests/spec_compiler/test_math_documentation_truthfulness.py` passed `8 passed`; `npm run test:frontend -- --run tests/frontend/workbench/app.test.js tests/frontend/workbench-ui.test.js` passed `48 passed`; `jq . docs/assets/readme/workbench/live-spy-30d/manifest.json` passed; `git diff --check` passed; forbidden exact-lane shorthand scan found only negative assertions; normalized saved SPY analysis produced an expanded `\hat{y}(t_n)=...` inverse-DFT formula with `has_old=False` and predictive search still `no_publishable_law`; screenshot SHA-256 values matched the manifest. | Final formula pass `APPROVE` | Replaced the remaining shorthand exact-lane label with a fully expanded real inverse-DFT equation using numeric coefficients, explicit row-index harmonics, and even-sample Nyquist handling while keeping exact reconstruction non-predictive and non-publishable. |
...
- Exact Reconstruction Math Juror: expanded real inverse-DFT formula rendering, odd/even sample sizes, tolerance, coefficient storage, finite-row requirements, target applicability.
