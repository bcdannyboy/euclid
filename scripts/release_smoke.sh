...
FULL_VISION_RUN_ARTIFACT="${PROJECT_ROOT}/build/certification/full_vision_run/run-result.json"
FULL_VISION_RUN_ID="$("${PYTHON_BIN}" -c 'import json, sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["run_id"])' "${FULL_VISION_RUN_ARTIFACT}")"
"${PYTHON_BIN}" -m euclid replay --run-id "${FULL_VISION_RUN_ID}" --output-root "${PROJECT_ROOT}/build/certification/full_vision_run" --evidence-report "${PROJECT_ROOT}/build/reports/full_vision_operator_replay_evidence.json"
"${PYTHON_BIN}" -m euclid release certify-clean-install --project-root "${PROJECT_ROOT}" --wheel-dir "${PROJECT_ROOT}/build/certification/clean_install/wheels" --output-root "${PROJECT_ROOT}/build/certification/clean_install"
