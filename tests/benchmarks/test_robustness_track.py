from __future__ import annotations

from pathlib import Path

import yaml

import euclid
from euclid.operator_runtime.run import run_operator

PROJECT_ROOT = Path(__file__).resolve().parents[2]
CURRENT_RELEASE_MANIFEST = PROJECT_ROOT / "examples" / "current_release_run.yaml"
FULL_VISION_MANIFEST = PROJECT_ROOT / "examples" / "full_vision_run.yaml"
POSITIVE_TASK = (
    PROJECT_ROOT / "benchmarks/tasks/robustness/robustness-medium-positive.yaml"
)
LEAKAGE_TASK = (
    PROJECT_ROOT / "benchmarks/tasks/robustness/robustness-medium-leakage.yaml"
)
SENSITIVITY_TASK = (
    PROJECT_ROOT
    / "benchmarks/tasks/robustness/robustness-medium-sensitivity-abstention.yaml"
)


def test_positive_case_requires_full_robustness_artifacts(tmp_path: Path) -> None:
    benchmark = euclid.profile_benchmark_task(
        manifest_path=POSITIVE_TASK,
        benchmark_root=tmp_path / "robustness-positive-benchmark",
        resume=False,
    )
    run = run_operator(
        manifest_path=CURRENT_RELEASE_MANIFEST,
        output_root=tmp_path / "robustness-positive-output",
    )
    graph = euclid.load_demo_run_artifact_graph(output_root=run.paths.output_root)
    run_result = graph.inspect(graph.root_ref).manifest
    robustness_report = graph.inspect(
        run_result.body["robustness_report_refs"][0]
    ).manifest

    assert benchmark.task_manifest.task_family == "robustness_positive"
    assert "null_result_ref" in robustness_report.body
    assert len(robustness_report.body["perturbation_family_result_refs"]) >= 2
    assert len(robustness_report.body["leakage_canary_result_refs"]) == 4
    assert len(robustness_report.body["sensitivity_analysis_refs"]) >= 1


def test_leakage_case_blocks_track(tmp_path: Path) -> None:
    benchmark = euclid.profile_benchmark_task(
        manifest_path=LEAKAGE_TASK,
        benchmark_root=tmp_path / "robustness-leakage-benchmark",
        resume=False,
    )
    manifest_path = _write_operator_manifest(
        tmp_path=tmp_path,
        request_id="robustness-track-leakage",
        robustness={
            "forced_leakage_canary_failures": ["future_target_level_feature"],
        },
    )

    run = run_operator(
        manifest_path=manifest_path,
        output_root=tmp_path / "robustness-track-leakage-output",
    )
    entry = euclid.publish_demo_run_to_catalog(output_root=run.paths.output_root)
    inspection = euclid.inspect_demo_catalog_entry(
        output_root=run.paths.output_root,
        publication_id=entry.publication_id,
    )

    assert benchmark.task_manifest.task_family == "robustness_leakage"
    assert entry.publication_mode == "abstention_only_publication"
    assert inspection.scorecard is not None
    assert (
        "leakage_canary_failed"
        in inspection.scorecard.manifest.body["descriptive_reason_codes"]
    )


def test_sensitivity_case_emits_abstention_mode(tmp_path: Path) -> None:
    benchmark = euclid.profile_benchmark_task(
        manifest_path=SENSITIVITY_TASK,
        benchmark_root=tmp_path / "robustness-sensitivity-benchmark",
        resume=False,
    )
    run = run_operator(
        manifest_path=CURRENT_RELEASE_MANIFEST,
        output_root=tmp_path / "robustness-sensitivity-output",
    )
    entry = euclid.publish_demo_run_to_catalog(output_root=run.paths.output_root)
    inspection = euclid.inspect_demo_catalog_entry(
        output_root=run.paths.output_root,
        publication_id=entry.publication_id,
    )
    graph = euclid.load_demo_run_artifact_graph(output_root=run.paths.output_root)
    run_result = graph.inspect(graph.root_ref).manifest
    robustness_report = graph.inspect(
        run_result.body["robustness_report_refs"][0]
    ).manifest

    assert benchmark.task_manifest.task_family == "robustness_sensitivity_abstention"
    assert entry.publication_mode == "abstention_only_publication"
    assert inspection.abstention is not None
    assert inspection.abstention.manifest.body["abstention_type"] == "robustness_failed"
    assert len(robustness_report.body["sensitivity_analysis_refs"]) >= 1


def _write_operator_manifest(
    *,
    tmp_path: Path,
    request_id: str,
    robustness: dict[str, object],
) -> Path:
    payload = yaml.safe_load(FULL_VISION_MANIFEST.read_text(encoding="utf-8"))
    assert isinstance(payload, dict)
    payload["request_id"] = request_id
    dataset_csv = Path(str(payload["dataset_csv"]))
    if not dataset_csv.is_absolute():
        payload["dataset_csv"] = str(
            (FULL_VISION_MANIFEST.parent / dataset_csv).resolve()
        )
    payload["robustness"] = robustness

    manifest_path = tmp_path / f"{request_id}.yaml"
    manifest_path.write_text(
        yaml.safe_dump(payload, sort_keys=False),
        encoding="utf-8",
    )
    return manifest_path
