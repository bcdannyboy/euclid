...
def test_portfolio_selected_candidate_runs_through_fit_and_point_prediction() -> None:
    ...
    fit_result = fit_candidate_window(
        candidate=portfolio_result.selected_candidate,
        feature_view=feature_view,
    ...
    assert portfolio_result.selection_record.selected_candidate_id == (
        "analytic_lag1_affine"
    )
    assert fit_result.family_id == "analytic"
    assert artifact.body["candidate_id"] == "analytic_lag1_affine"
    assert artifact.body["forecast_object_type"] == "point"
    assert artifact.body["missing_scored_origins"] == []


def test_portfolio_winner_has_replayable_comparison_universe() -> None:
    feature_view, audit = _feature_view()
    evaluation_plan = build_evaluation_plan(
        feature_view=feature_view,
        audit=audit,
    ...
    assert len(portfolio_result.selection_record.compared_finalists) >= 3
    assert portfolio_result.selection_record.selected_candidate_id is not None
    assert portfolio_result.selection_record.decision_trace[-1]["step"] == (
...
def _feature_view():
    snapshot = FrozenDatasetSnapshot(
        series_id="phase06-portfolio-series",
...
def _score_policy_manifest(catalog, evaluation_plan):
    ...
    return ManifestEnvelope.build(
        ...
        module_id="scoring",
        body={
            "score_policy_id": "phase06_portfolio_prediction_policy_v1",
