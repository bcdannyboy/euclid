...
The Evidence Studio displays sanitized live evidence, replay artifact links, claim lane, downgrade and abstention reasons, engine provenance, fitting diagnostics, scoring diagnostics, falsification diagnostics, and whether evidence is non-claim support. It redacts secrets before data reaches the browser. Live API success is displayed as provider/runtime evidence only; it cannot become scientific claim evidence without the runtime claim, replay, and readiness contracts.

The display surface includes `descriptive_fit`, `predictive_law`, `holistic_equation`, `uncertainty_attachment`, replay links, and live-evidence boundaries. Missing or malformed evidence is shown as a lower claim ceiling or abstention reason rather than an implicit promotion.
